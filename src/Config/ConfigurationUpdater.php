...
final class ConfigurationUpdater
{
     ...
     *
     * @param array{enabled:bool,front:bool,front_ajax:bool,ttl:int,prefix:string,compression:bool,compression_auto:bool,compression_threshold:int,serializer:string} $data
     */
    public function saveCache(array $data): void
    {
        ...
        // Redis is strictly dedicated to the front office. Back office, CLI and
        // cron are always forced OFF (their toggles were removed from the UI:
        // enabling them is pointless and risky).
        $this->configuration->set(ContextResolver::KEY_BACK, 0);
        $this->configuration->set(ContextResolver::KEY_CLI, 0);
        $this->configuration->set(ContextResolver::KEY_CRON, 0);
